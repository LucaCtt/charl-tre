...
def _split_mats(mats: list[np.ndarray], test_ratio: float, n_parts: int) -> tuple[list[np.ndarray], list[np.ndarray]]:
    ...
    """
    if test_ratio == 0:
        return mats, [np.empty((0, *mat.shape[1:])) for mat in mats]  # Return empty test mats if test_ratio is 0

    if test_ratio == 1:
        return [np.empty((0, *mat.shape[1:])) for mat in mats], mats  # Return empty train mats if test_ratio is 1
