...
class CSIAugmenter:
    ...
    def __init__(
        ...
        self.__augmentation_prob = augmentation_prob
        self.__rng = np.random.default_rng(seed)

    def __gaussian_noise(self, x: np.ndarray) -> np.ndarray:
        """Simulate thermal noise in the WiFi hardware."""
        noise = self.__rng.normal(0, self.__noise_std, x.shape)
        noisy = x + noise
        return np.clip(noisy, 0.0, 1.0).astype(np.float32)

    def __subcarrier_masking(self, x: np.ndarray) -> np.ndarray:
        """Randomly masks frequency subcarriers (columns) to simulate interference."""
        mask = self.__rng.random(x.shape[-1]) > self.__mask_prob
        # Broadcast mask across antennas and time
        return x * mask[np.newaxis, np.newaxis, :]

    def __time_warp(self, x: np.ndarray) -> np.ndarray:
        """Randomly stretches or compresses the time dimension."""
        c, t, s = x.shape
        # Create a random warp factor
        warp_factor = self.__rng.uniform(0.9, 1.1)
    ...
    def __temporal_block_out(self, x: np.ndarray) -> np.ndarray:
        ...
        block_size = int(self.__rng.integers(1, max_block + 1))
        start = int(self.__rng.integers(0, t - block_size + 1))
    ...
    def apply(self, x: np.ndarray) -> np.ndarray:
        ...
        if self.__rng.random() < self.__augmentation_prob:
            x = self.__gaussian_noise(x)
        if self.__rng.random() < self.__augmentation_prob:
            x = self.__subcarrier_masking(x)
        if self.__rng.random() < self.__augmentation_prob:
            x = self.__time_warp(x)
        if self.__rng.random() < self.__augmentation_prob:
            x = self.__temporal_block_out(x)
