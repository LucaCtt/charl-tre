"""Trainer for early fusion Dirichlet VAE."""
...
class TrainerParams(TypedDict):
    """Parameters for configuring the early fusion trainer."""
...
class Trainer:
    """Trainer class for early fusion VAE using Distributed Data Parallel (DDP)."""
    ...
    def __init__(
        ...
        Arguments:
            model: Early fusion VAE model to be trained.
    ...
    def _run_batch(
        ...
        Arguments:
            x_true: Ground truth input tensor of shape (batch_size, n_antennas, window_size, n_subcarriers).
            kl_weight: Current KL divergence weight.
            free_bits: Current free-bits floor for KL divergence.

        Returns:
            Tuple containing total loss, reconstruction loss, KL divergence loss, and concat_alpha.

        """
        self._optimizer.zero_grad(set_to_none=True)

        with torch.autocast(device_type=self._device.type, dtype=torch.float16):
            recon, alpha = self._model(x_true)

            # Compatibility: allow either list[Tensor] or already-stacked tensor.
            x_recon = torch.stack(recon, dim=1) if isinstance(recon, list) else recon
    ...
    def _run_val_epoch(self, kl_weight: float, free_bits: float) -> torch.Tensor:
        ...
        for x_true_cpu, _ in self._val_dl:
            ...
            with torch.autocast(device_type=self._device.type, dtype=torch.float16):
                reconstructions, concat_alpha = self._model(x_true)
                x_recon = torch.stack(reconstructions, dim=1) if isinstance(reconstructions, list) else reconstructions
                loss, _, _ = elbo_loss(
                    x_recon,
                    x_true,
                    concat_alpha,
    ...
    def train(self, epochs: int) -> tuple[float, float, float]:
        """Train the early fusion VAE model for a specified number of epochs.
