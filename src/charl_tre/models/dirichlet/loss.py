...
def elbo_loss(
    ...
    Arguments:
        x_recon (torch.Tensor): Reconstructed input of shape (batch_size, ...)
        x_true (torch.Tensor): Ground truth input of shape (batch_size, ...)
        ...
        free_bits (float): Number of free bits to allow in the KL divergence (default: 0.0)

    Returns:
        tuple[torch.Tensor, torch.Tensor, torch.Tensor]: A tuple containing:
            - total_loss (torch.Tensor): Combined ELBO loss
    ...
    # KL divergence loss
    kl_per_sample = _kl_divergence(alpha, free_bits)
...
def _kl_divergence(alpha: torch.Tensor, free_bits: float = 0.0) -> torch.Tensor:
    ...
    Returns:
        kl_loss (torch.Tensor): KL divergence averaged over batch of shape (batch_size,)
